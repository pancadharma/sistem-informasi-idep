...
$(document).ready(function() {
    ...
    map.on('click', function(e) {
        ...
        <div>
            ...
            Latitude: ${lat}<br>
            Longitude: ${lng}<br>
            <button class="btn btn-primary btn-sm use-coordinates" data-lat="${lat}" data-lng="${lng}">Use Coordinates</button>
        </div>
        `;
        clickMarker.bindPopup(popupContent).openPopup();
    });

    // Handle the use coordinates button click
    $(document).on('click', '.use-coordinates', function(e) {
        e.preventDefault();
        const lat = $(this).data('lat');
        const lng = $(this).data('lng');
        useCoordinates(lat, lng, e);
    });
