<div id="add_dusun" class="card card-primary">
    ...
    <div class="card-body">
        <form id="submit_dusun" action="{{ route('api.beneficiary.dusun.simpan')}}" method="POST" autocomplete="off" novalidate>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" name="provinsi_id" id="provinsi_id" required style="width: 100%"></select>
            </div>

            <div class="form-group">
                <label for="kabupaten_id">{{ __('cruds.dusun.form.kab') }}</label>
                <select id="kabupaten_id" name="kabupaten_id" class="form-control select2" required style="width: 100%">
            ...
            <div class="form-group">
                ...
                <select id="kecamatan_id" name="kecamatan_id" class="form-control select2" required style="width: 100%">
            ...
            <div class="form-group">
                ...
                <select id="desa_id_dusun" name="desa_id_dusun" class="form-control select2" required style="width: 100%">
            ...
            <div class="form-group">
                ...
                <input type="text" id="nama" name="nama" class="form-control" required maxlength="200" pattern="^[A-Za-z][A-Za-z0-9 .]*$" title="Must start with a letter.">
            </div>

            <div class="form-group">
                <label for="kode_pos">{{ __('cruds.dusun.form.kode_pos') }}</label>
                <input type="text" id="kode_pos" name="kode_pos" class="form-control" maxlength="5" pattern="\d*" title="Only numbers allowed">
            ...
            <button class="btn btn-success float-right btn-add-dusun" type="submit"><i class="fas fa-save"></i> {{ trans('global.save') }}</button>
        </form>
    </div>
</div>
...
$(document).ready(function() {
    ...
    $('#submit_dusun').on('submit', function(e) {
        ...
        let kode_desa = $('#desa_id_dusun').find('option:selected').data('id') || $('#kode_desa').val() || '';
    ...
    $('#provinsi_id').change(function() {
        ...
        if (provinsi_id) {
            ...
            $.ajax({
                url: "{{ route('api.kab', ':id') }}".replace(':id', provinsi_id), // Replace with your kabupaten API route
                type: 'GET',
                dataType: 'json',
                beforeSend: function() {
                    Toast.fire({
                ...
                success: function(data) {
                    $('#kabupaten_id').empty();
                    $('#kabupaten_id').append('<option value="" selected>{{ trans('global.pleaseSelect') }} {{ trans('cruds.kabupaten.title')}}</option>'); // Add default option
                    $.each(data, function(key, value) {
                        $('#kabupaten_id').append('<option value="' + value.id + '" data-id="' + value.kode + '">' + value.text + '</option>'); // Assuming API returns id and text
                    });
                    $('#kabupaten_id').trigger('change'); // Trigger change event to re-initialize Select2 (important!)
                },
                error: function(xhr, status, error) {
    ...
    $('#kabupaten_id').change(function() {
        ...
        if (kabupaten_id) {
            $.ajax({
                ...
                success: function(data) {
                    $('#kecamatan_id').empty();
                    $('#kecamatan_id').append('<option value="" selected>{{ trans('global.pleaseSelect') }} {{ trans('cruds.kecamatan.title')}}</option>'); // Add default option
                    $.each(data, function(key, value) {
                        $('#kecamatan_id').append('<option value="' + value.id + '" data-id="' + value.kode + '">' + value.text + '</option>'); // Assuming API returns id and text
                    });
    ...
    $('#kecamatan_id').change(function() {
        ...
        if (kecamatan_id) {
            ...
            $.ajax({
                url: "{{ route('api.desa', ':id') }}".replace(':id', kecamatan_id), // Replace with your desa API route
                type: 'GET',
                dataType: 'json',
                beforeSend: function() {
                    Toast.fire({
                ...
                success: function(data) {
                    $('#desa_id_dusun').empty();
                    $('#desa_id_dusun').append('<option value="" selected>{{ trans('global.pleaseSelect') }} {{ trans('cruds.desa.title')}}</option>'); // Add default option
                    $.each(data, function(key, value) {
                        $('#desa_id_dusun').append('<option value="' + value.id + '" data-id="' + value.kode + '">' + value.text + '</option>'); // Assuming API returns id and text
                    });
            ...
            if (xhr.responseJSON.message) {
                ...
                $.each(xhr.responseJSON.errors, function(field, messages) {
                    $.each(messages, function(index, message) {
                        errorMessage += "<li>" + (field ? field + ": " : "") + message + "</li>"; // Show field name
                    });
                });
                errorMessage += "</ul>";
            }
