<script>
    ...
    $(document).ready(function() {
        $('#kaitan_sdg_list').DataTable({
            responsive: true,
            ajax: "{{ route('data.kaitan_sdg') }}",  // Update the route for kaitan_sdg data
            processing: true,
            serverSide: true,
            columns: [
                {
        ...
        $('#editKaitanSdgForm').on('submit', function(e) {
            e.preventDefault();
            let id = $('#id').val();
            let url = '{{ route('kaitan_sdg.update', ':id') }}'.replace(':id', id);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('#editKaitanSdgModal').modal('hide');
                        $('#kaitan_sdg_list').DataTable().ajax.reload();
                    }
                },
                error: function(xhr) {
    ...
    $(document).ready(function() {
        ...
        $('#editKaitanSdgForm').submit(function(e) {
            e.preventDefault();
            let idKaitanSdg = $('#id').val(); // Get the ID of the Kaitan SDG
            let formData = $(this).serialize(); // Serialize the form data
            // AJAX request for updating Kaitan SDG
            $.ajax({
                url: '{{ route('kaitan_sdg.update', ':id_p') }}'.replace(':id_p', idKaitanSdg), // Replace the placeholder with the actual ID
                method: 'PUT',
                dataType: 'JSON',
                data: formData,
                success: function(response) {
                    if (response.success === true) {  // Check the `success` key, not `status`
                        ...
                        $('#editKaitanSdgModal').modal('hide'); // Hide the modal
                        $('#editKaitanSdgForm').trigger('reset'); // Reset the form
                        $('#kaitan_sdg_list').DataTable().ajax.reload(); // Reload the DataTable
    ...
    $(document).ready(function() {
        ...
        $('.btn-add-kaitan-sdg').on('click', function(e) {
            e.preventDefault();
            var formDataKaitanSdg = $('#kaitanSdgForm').serialize(); // Serialize form data
            $.ajax({
                method: "POST",
                url: '{{ route('kaitan_sdg.store') }}', // Route for storing Kaitan SDG
                data: formDataKaitanSdg,
                dataType: 'json',
                success: function(response) {
                    if (response.success === true) {
                        ...
                        $('#addKaitanSdg').modal('hide'); // Hide modal after success
                        $('#kaitanSdgForm').trigger('reset'); // Reset form fields
                        $('#kaitan_sdg_list').DataTable().ajax.reload(); // Reload DataTable to reflect changes
                    } else {
    ...
    //View Data
    $(document).ready(function() {
        $('#kaitan_sdg_list tbody').on('click', '.view-kaitansdg-btn', function(e) {
            e.preventDefault();
            let kaitanSdgId = $(this).data('kaitansdg-id'); // Get the ID for the kaitan_sdg
            let action = $(this).data('action'); // Get the action, in this case 'view'
            $.ajax({
                url: '{{ route('kaitan_sdg.show', ':id') }}'.replace(':id', kaitanSdgId), // Route with ID placeholder
                method: 'GET',
                dataType: 'json',
                success: function(response) {
                    let data = response || [];
                    if (action === 'view') {
                        ...
                        $('#showKaitanSdgModal').modal('show'); // Show the modal
                    } else {
                        Swal.fire({
                            text: "Error",
    ...
    // Call Modal Edit
    $(document).ready(function() {
        $('#kaitan_sdg_list tbody').on('click', '.edit-kaitansdg-btn', function(e) {
            e.preventDefault();
            let kaitanSdgId = $(this).data('kaitansdg-id'); // Get the ID of the Kaitan SDG
            let newActionUrl = '{{ route('kaitan_sdg.update', ':id') }}'.replace(':id', kaitanSdgId); // Update the action URL dynamically
            // Make an AJAX call to get the data for the selected Kaitan SDG
            $.ajax({
                url: '{{ route('kaitan_sdg.edit', ':id') }}'.replace(':id', kaitanSdgId),
                method: 'GET',
                dataType: 'json',
                success: function(response) {

                    $("#editKaitanSdgForm").trigger('reset'); // Reset the form fields
                    $("#editKaitanSdgForm").attr("action", newActionUrl); // Set the form action URL
                    ...
                    $('#editKaitanSdgModal').modal('show'); // Show the edit modal
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    Swal.fire({
                        title: 'Error!',
