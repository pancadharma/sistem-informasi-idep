<script>
    ...
    let rowCount = 0
    const editDataForm = null // Initialize as null
    ...
    function editRow(row) {
      ...
      const kelompokRentanFullData = JSON.parse(
        currentRow.find("td[data-kelompok_rentan]").attr("data-kelompok_rentan_full") || "[]",
      )
      ...
      // Populate the select2 with existing data
      kelompokRentanFullData.forEach((item) => {
        var option = new Option(item.text, item.id, true, true)
        $("#editKelompokRentan").append(option)
      })
      $("#editKelompokRentan").trigger("change")
