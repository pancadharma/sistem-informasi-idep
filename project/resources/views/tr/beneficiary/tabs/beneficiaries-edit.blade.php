...
<div class="row">
    <div class="col-sm-12 col-md-12 col-lg-4 self-center order-1 order-md-1">
        ...
        <!-- id program -->
        <input type="hidden" name="program_id" id="program_id" value="{{ $program->id }}" readonly>
        <input type="hidden" name="user_id" id="user_id" value="{{ Auth::id() }}">
        <!-- kode program -->
        <input type="text" class="form-control" id="kode_program" placeholder="{{ __('cruds.kegiatan.basic.program_select_kode') }}" name="kode_program" value="{{ $program->kode }}"
        data-toggle="modal" data-target="#ModalDaftarProgram" disabled readonly>
    </div>
    <!-- nama program-->
    <div class="col-sm-12 col-md-12 col-lg-8 self-center order-2 order-md-2">
        ...
        <input type="text" class="form-control" id="nama_program" placeholder="{{ __('cruds.kegiatan.basic.program_nama') }}" name="nama_program" value="{{ $program->nama }}" readonly>
    </div>
</div>
...
<div class="row">
    <div class="col-12">
        @foreach ($data as $item)
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">{{ $item->jenisKelompok }}</h5>
...
{{--
    <h1>{{ __('Edit Beneficiaries for Program') }}: {{ $program->nama }}</h1> --}}
                ...
                <tbody id="tableBody" class="display nowrap overflow-auto">
                    @foreach ($beneficiaries as $beneficiary)
                        <tr data-id="{{ $beneficiary->id }}">
                            ...
                            <td data-gender="{{ $beneficiary->jenis_kelamin }}">{{ $beneficiary->jenis_kelamin === 'laki' ? 'Laki-laki' : ($beneficiary->jenis_kelamin === 'perempuan' ? 'Perempuan' : 'Lainnya') }}</td>
                            <td data-kelompok_rentan="{{ $beneficiary->kelompokMarjinal->pluck('id')->implode(',') }}">{{ $beneficiary->kelompokMarjinal->pluck('nama')->implode(', ') }}</td>
                            <td data-rt="{{ $beneficiary->rt }}">{{ $beneficiary->rt }}</td>
                            <td data-rw="{{ $beneficiary->rw }}">{{ $beneficiary->rw }}</td>
                            <td data-dusun-id="{{ $beneficiary->dusun_id }}">{{ $beneficiary->dusun_id }}</td>
                            <td data-desa-id="{{ $beneficiary->desa_id ?? '' }}">{{ $beneficiary->desa_id ?? '' }}</td>
                            <td data-no_telp="{{ $beneficiary->no_telp }}">{{ $beneficiary->no_telp }}</td>
                            <td data-jenis_kelompok="{{ $beneficiary->jenisKelompok->pluck('id')->implode(',') }}">{{ $beneficiary->jenisKelompok->pluck('nama')->implode(', ') }}</td>
                            ...
                            <td data-activity-ids="{{ $beneficiary->penerimaActivity->pluck('id')->implode(',') }}">{{ $beneficiary->penerimaActivity->pluck('nama')->implode(', ') }}</td>
