<!-- Hasil Kegiatan -->
<div class="form-group row">
    <label for="hasil_kegiatan" class="col-sm-3 col-md-3 col-lg-2 order-1 order-md-1 col-form-label">{{ __('cruds.kegiatan.hasil.label') }}</label>
    <div class="col-sm col-md col-lg order-2 order-md-2 self-center">
        <textarea name="hasil_kegiatan" id="hasil_kegiatan" class="form-control summermnote" rows="2"
        placeholder=" {{ __('cruds.kegiatan.hasil.label') }}"></textarea>
   </div>
</div>
<!-- Rencana Tindak Lanjut -->
<div class="form-group row">
    <label for="tindak_lanjut" class="col-sm-3 col-md-3 col-lg-2 order-1 order-md-1 col-form-label">{{ __('cruds.kegiatan.hasil.tindak_lanjut') }}</label>
    <div class="col-sm col-md col-lg order-2 order-md-2 self-center">
        <textarea name="tindak_lanjut" id="tindak_lanjut" class="form-control summermnote" rows="2"
        placeholder=" {{ __('cruds.kegiatan.hasil.tindak_lanjut') }}"></textarea>
   </div>
</div>
<!-- Rencana Tindak Lanjut -->
<div class="form-group row">
    <label for="tantangan" class="col-sm-3 col-md-3 col-lg-2 order-1 order-md-1 col-form-label">{{ __('cruds.kegiatan.hasil.tantangan') }}</label>
    <div class="col-sm col-md col-lg order-2 order-md-2 self-center">
        <textarea name="tantangan" id="tantangan" class="form-control summermnote" rows="2"
        placeholder=" {{ __('cruds.kegiatan.hasil.tantangan') }}"></textarea>
   </div>
</div>
