    ...
    <div class="row">
        <div class="col-lg-4 form-group">
            <div class="input-group">
                ...
                <textarea type="textarea" id="goals_deskripsi" name="goals_deskripsi" class="form-control {{ $errors->has('goals_deskripsi') ? 'is-invalid' : '' }}" placeholder=" {{ __('cruds.program.outcome.desc') }}" rows="5" maxlength="1000">{{ old('goals_deskripsi', $program->goal->deskripsi ?? '') }}</textarea>
            </div>
        </div>
        <div class="col-lg-4 form-group">
            <div class="input-group">
                ...
                <textarea id="indikator" name="goals_indikator" cols="30"
                    ...
                    placeholder="{{ __('cruds.program.outcome.indicator') }}" maxlength="1000" rows="5">{{ old('goals_indikator', $program->goal->indikator ?? '') }}</textarea>
            </div>
        </div>
        <div class="col-lg-4 form-group">
            <div class="input-group">
                ...
                <textarea id="goals_target" name="goals_target" cols="30"
                    ...
                    placeholder="{{ __('cruds.program.outcome.target') }}" maxlength="1000" rows="5">{{ old('goals_target', $program->goal->target ?? '') }}</textarea>
            </div>
        </div>
    </div>
</div>
