        ...
        <div class="card-body table-responsive">
            <form action="{{ route('trprogram.update.doc', [$program->id]) }}" id="upload-form" method="POST"
                ...
                <div class="form-group">
                    <label for="file_pendukung">Select Files</label>
                    <div class="file-loading">
                        <input id="file_pendukung" name="file_pendukung[]" type="file" multiple>
                    </div>
...
<script>
    ...
    //         },
    //         success: function(response) {
    //             console.log('Deleted Files Success:', response);
    //             $('#file_pendukung').on('fileremoved', function(event, id, index) {
    //                 initializeFileInput();
    //                 console.log
    //             });
    //             if (response.success) {
    ...
    <script>
        function fetchMediaAndInitFileInput() {
            let programId = {{ $program->id }};
            let url = '{{ route('trprogram.getMedia', ':id') }}'.replace(':id', programId);
            $.ajax({
                url: url,
        ...
        function initializeFileInput(response) {
            var fileIndex = 0;
            var fileCaptions = {};
            ...
            $("#file_pendukung").fileinput({
                ...
                browseOnZoneClick: true,
                showRemove: false,
                allowedFileExtensions: ['jpg', 'png', 'jpeg', 'docx', 'doc', 'ppt', 'pptx', 'xls', 'xlsx', 'csv',
                    'gif', 'pdf'
                ],
                maxFileSize: 10000,
                maxFilePreviewSize: 2048,
                ...
                previewFileIconSettings: {
                    ...
                    'webp': '<i class="bi bi-file-earmark-image"></i>',
                },
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.kv-file-remove', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        console.log('Deleted Files Success:', response.success)
                        ...
                        if (response.success) {
                            ...
                            $('#file_pendukung').on('fileloaded', function(event) {
                                fetchMediaAndInitFileInput();
                                console.log("trigger loaded file after delete success")
                            });
                            fetchMediaAndInitFileInput();
                            console.log('fetchMediaAndInitFileInput called after delete success');
    ...
    {{-- <script>
        ...
        function deleteMedia(mediaID) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    Swal.fire({
                        title: "Error",
                        text: `Error: ${xhr.statusText}. Status code: ${xhr.status}. Response: ${xhr.responseText}`,
                        icon: 'error',
                        timer: 5000,
                    });
                }
        ...
        function loadMedia() {
            ...
            $.getJSON(url, function(response) {
                fileInputData(response); // Call fileInputData with the response as an argument
            }).fail(function(xhr) {
                console.error('AJAX DATA ERR : ', xhr);
            });
        }
